...
def load_data():
    '''
    Loading the data and reformatting for use of ANOVA.
    :return:
    '''
...
def preform_anova(df_anova, post_hoc = False):
    ...
    aov_table = sm.stats.anova_lm(model, typ=2) # Type 2 is for ANOVA DataFrame
...
def main():
    df_anova = load_data()
    preform_anova(df_anova, False)
    preform_multilinear_regression(df_anova)
    plt.show()
